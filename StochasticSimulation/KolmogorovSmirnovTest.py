...
df2 = df2.iloc[index]
def simulate_random_match(num_points, serve_win_prob=0.68):
...
# 模拟N次随机比赛
N = 1000
simulated_scores = [simulate_random_match(len(actual_momentum_scores_diff)) for _ in range(N)]
# 统计检验
# 比较实际势头得分与模拟得分的分布差异
# 这里使用Kolmogorov-Smirnov test作为示例
ks_stat, p_value = stats.ks_2samp(actual_momentum_scores_diff, np.concatenate(simulated_scores))
