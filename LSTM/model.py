...
true_data = pd.read_excel('../data/standard_data.xlsx')
...
if Train:
    ...
    torch.save(lstm_model.state_dict(), '../models/save_model.pth')
    ...
    plt.ylabel('Error')
    # 保存图表到本地
    plt.savefig('../Pictures/training_error.png')
else:
    # 加载模型进行预测
    lstm_model.load_state_dict(torch.load('../models/save_model.pth'))
    ...
    plt.ylabel('value')
    plt.legend()
    plt.savefig('../Pictures/test——results.png')