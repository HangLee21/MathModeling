...
if Train:
    ...
    df = df.iloc[index]
    # 加载已保存的模型
    model = load_model('../models/simple_model_1_momentum_reversal.keras', compile=False, safe_mode=False)
    # 准备测试数据
    ...
    plt.xlabel('Point')
    plt.ylabel('Value')
    text = f'Accuracy: {accuracy}'  # 替换为实际要显示的文字
    # 设置文字的位置坐标
    x = fig_width * 0.9  # 图像宽度的0.9，即右边留出10%的空间
    y = fig_height * 0.9  # 图像高度的0.9，即上边留出10%的空间

    # 添加文字到图像的右上角
    plt.text(x, y, text, ha='right', va='top', fontsize=12)
    # 显示图形
    plt.show()
